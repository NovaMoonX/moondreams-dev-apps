...
function normalizeRequestQueue(value: unknown): Array<{ userId: string; method: string }> {
...
async function validatePresenceForMembers(
  ...
  for (const memberUid of members) {
    ...
    const currentLocation =
      typeof status.currentLocation === 'string' ? status.currentLocation : null;
...
export const triggerBoxAction = onCall({ region: 'us-central1' }, async (request) => {
  const authUid = request.auth?.uid;

  if (!authUid) {
    throw new HttpsError('unauthenticated', 'You must be signed in to trigger a reveal.');
  }
  ...
  const boxRef = firestore.doc(`${APP_PATH}/spaces/${spaceId}/boxes/${boxId}`);

  const [spaceSnapshot, boxSnapshot] = await Promise.all([spaceRef.get(), boxRef.get()]);
  ...
  if (!boxSnapshot.exists) {
    throw new HttpsError('not-found', `Box ${boxId} does not exist in space ${spaceId}.`);
  }
  ...
  if (!members.includes(authUid)) {
    throw new HttpsError('permission-denied', 'You are not a member of this Worth the Wait space.');
  }

  await validatePresenceForMembers(realtimeDb, members);
  assertMutualRequests(boxSnapshot.data()?.revealRequestedBy, members, method);
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    if (currentAction && currentAction.status !== 'completed') {
      throw new HttpsError('already-exists', 'A reveal is already in progress.');
    }
  ...
  const unrevealedItemsSnapshot = await boxRef.collection('items').where('isRevealed', '==', false).get();
  ...
  if (method === 'raffle') {
    if (unrevealedItems.length === 0) {
      throw new HttpsError('failed-precondition', 'No unrevealed items remain to raffle.');
    }
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    const currentAction = (spaceTx.data()?.activeAction ?? null) as IndexedActionRecord | null;
