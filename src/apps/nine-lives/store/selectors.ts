...
export const selectHealthRecordsByCat = (catId: string | null | undefined) => (state: RootState) =>
  catId ? state.nineLives.healthRecords.items.filter((record) => record.catId === catId) : [];
