...
function getPreventiveTypeLabel(
  preventive: Preventive,
  customTypes: CustomPreventiveType[],
): string {
  if (preventive.type === 'custom') {
    const customType = customTypes.find(
      (type) => type.id === preventive.customTypeId,
    );
...
function PreventiveTimeline({
  ...
  const { page, pageCount, setPage, pagedItems, shouldPaginate } =
    usePagination(sortedPreventives, PAGE_SIZE);

  if (sortedPreventives.length === 0) {
    return (
      <p className='text-muted-foreground text-sm'>
        No preventive doses logged yet.
      </p>
    );
  }
          ...
          return (
            ...
            >
              <div className='min-w-0'>
                ...
                <div className='text-muted-foreground text-sm'>
                  Last administered:{' '}
                  {formatDateTime(preventive.lastAdministeredAt)}
                </div>
              ...
              {!readOnly && menuItems.length > 0 && (
                <EllipsisDropdown
                  ...
                  ariaLabel={`Open actions for ${preventive.name}`}
                />
              )}
            </div>
          );
